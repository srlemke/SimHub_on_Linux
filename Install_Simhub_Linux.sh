...
if [ "$install_simhub" = "y" ] || [ "$install_simhub" = "Y" ]; then
    ...
    echo "2. Enable only the bellow 2: (You can enable others if you have such devices, just make sure to no intall dotnet via SimHub installer) "
    echo "   - Open Windows Firewal port, this is for web browser and handy access."
    echo "   - Defaults dashes (if you want them)"
    echo "3. Do not run SimHub from the installer at the end, unmark that option."
    echo "   - The reason is it will lock the game prefix and you wont be able to start the game."
    echo "   - In case you did run it, Reboot or killing SimHub process fixes this."
    echo ""
    echo "=========================================="
    echo ""
